...
function renderTagStats(){
  ...
  let html=`<div class="stats-grid"><div class="stat-card"><div class="stat-num">${total}</div><div class="stat-lbl">筆記總數</div></div><div class="stat-card"><div class="stat-num">${Object.keys(lnk).length}</div><div class="stat-lbl">有關聯筆記</div></div><div class="stat-card"><div class="stat-num">${domains.length}</div><div class="stat-lbl">數</div></div></div><div class="usage-time">已使用 KLaws：${usageText}</div>`;
  html+=`<div style="font-size:11px;font-weight:700;color:#888;margin:10px 0 6px;">各筆記數</div>`;
