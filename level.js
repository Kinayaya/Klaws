...
function renderGroupPathList() {
  ...
  if(pathSearchQ) list=list.filter(item=>matchesExactQuery({query:pathSearchQ,candidates:[item.label||'',item.key||'',subByKey(item.domain).label]}));
...
function renderPartPathList() {
  ...
  if(pathSearchQ) list=list.filter(item=>matchesExactQuery({query:pathSearchQ,candidates:[item.label||'',item.key||'',groupByKey(item.group).label]}));
  if(!list.length){el.innerHTML='<div style="color:#bbb;font-size:13px;padding:8px 0">（無符合條件的）</div>';return;}
  el.innerHTML=list.map(item=>{
