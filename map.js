(function (global) {
  const MAP_NODE_RADIUS_MIN = 15;
  const MAP_NODE_RADIUS_MAX = 100;
  const MAP_NODE_RADIUS_DEFAULT = 25;
  const MAP_LIGHT_BUNDLING_STRENGTH = 0.38;
  const DEFAULT_LANE_NAMES = ['法條', '構成要件', '違法性', '罪責', '其它'];
  const MIN_LANE_COUNT = 2;
  const MAX_LANE_COUNT = 10;
  const clampMapRadius = (radius) => Math.max(MAP_NODE_RADIUS_MIN, Math.min(MAP_NODE_RADIUS_MAX, radius));
  const defaultLaneNameAt = (idx) => DEFAULT_LANE_NAMES[idx] || `泳道 ${idx + 1}`;
  const normalizeLaneCount = (value) =>
    Math.max(MIN_LANE_COUNT, Math.min(MAX_LANE_COUNT, parseInt(value, 10) || DEFAULT_LANE_NAMES.length));
  const splitMapTitleLines = (title, max = 8) => {
